...
Before dispatching the materialized local roster, read `references/orca-routing.md` and `references/orca-review-dispatch.md`. Send only the already-selected local reviewer nodes whose stage resolves to Orca, using the complete prompts and context defined by this reference, and do not also launch those nodes through the foreground native queue. Keep the fast pass, cross-model peer process, roster selection, merge, validation selection, fixes, synthesis, and every write in this controller. For native routing, continue with the foreground queue unchanged.
